...
Game::Game(int rows, int cols, int nRabbits) {
  ...
  if (nRabbits > MAXRABBITS) {
    cout << "***** Trying to create Game with " << nRabbits << " rabbits; only "
         << MAXRABBITS << " are allowed!" << endl;
    exit(1);
  }
  int nEmpty = rows * cols - nRabbits - 1; // 1 for Player
  if (nEmpty < 0) {
    cout << "***** Game created with a " << rows << " by " << cols
         << " arena, which is too small to hold a player and " << nRabbits
         << " rabbits!" << endl;
    exit(1);
  }
  ...
  for (;;) {
    ...
    /// need to complete
      ...
      if (tolower(playerMove[0]) == 'c')
        ...
        cout << "Player move must be nothing, or 1 character n/e/s/w/c/h."
             << endl;
      }
    }
  }
